...
bool AppInit2(int argc, char* argv[])
{
    ...
    if (mapArgs.count("-?") || mapArgs.count("--help"))
    {
        string strUsage = string() +
          ...
          _("Usage:") + "\t\t\t\t\t\t\t\t\t\t\n" +
            "  runnercoin [options]                   \t  " + "\n" +
            "  runnercoin [options] <command> [params]\t  " + _("Send command to -server or runnercoind") + "\n" +
            "  runnercoin [options] help              \t\t  " + _("List commands") + "\n" +
            "  runnercoin [options] help <command>    \t\t  " + _("Get help for a command") + "\n";
            
        strUsage += "\n" + HelpMessage();

#if defined(__WXMSW__) && defined(GUI)
        // Tabs make the columns line up in the message box
        wxMessageBox(strUsage, "Runnercoin", wxOK);
#else
        // Remove tabs
    ...
    {
        wxMessageBox("Incorrect -algo parameter specified, expected sha256d or scrypt", "Runnercoin");
        return false;
    }
...
//#if defined(__WXMSW__) && defined(GUI)
    ...
    if (!lock.try_lock())
    {
        wxMessageBox(strprintf(_("Cannot obtain a lock on data directory %s.  Runnercoin client is probably already running."), GetDataDir().c_str()), "Runnercoin");
        return false;
    }
    ...
    if (!fNoListen)
    {
        ...
        {
            wxMessageBox(strErrors, "Runnercoin");
            return false;
        }
    }
    ...
    //
    if (fDaemon)
        fprintf(stdout, "runnercoin server starting\n");
    strErrors = "";
    int64 nStart;
    ...
    if (mapArgs.count("-mininput"))
    {
        ...
        {
            wxMessageBox(_("Invalid amount for -mininput=<amount>"), "Runnercoin");
            return false;
        }
    }
    ...
    if (!strErrors.empty())
    {
        wxMessageBox(strErrors, "Runnercoin", wxOK | wxICON_ERROR);
        return false;
    }
    ...
    if (mapArgs.count("-proxy"))
    {
        ...
        {
            wxMessageBox(_("Invalid -proxy address"), "Runnercoin");
            return false;
        }
    }
    ...
    if (mapArgs.count("-paytxfee"))
    {
        ...
        {
            wxMessageBox(_("Invalid amount for -paytxfee=<amount>"), "Runnercoin");
            return false;
        }
        if (nTransactionFee > 0.25 * COIN)
            wxMessageBox(_("Warning: -paytxfee is set very high.  This is the transaction fee you will pay if you send a transaction."), "Runnercoin", wxOK | wxICON_EXCLAMATION);
    }
    ...
    if (!CreateThread(StartNode, NULL))
        wxMessageBox("Error: CreateThread(StartNode) failed", "Runnercoin");
...
std::string HelpMessage()
{
    std::string strUsage = std::string(_("Options:\n")) +
        "  -conf=<file>     \t\t  " + _("Specify configuration file (default: runnercoin.conf)\n") +
        "  -pid=<file>      \t\t  " + _("Specify pid file (default: runnercoind.pid)\n") +
