...
class CRunnercoinHooks : public CHooks
{
public:
...
string SendMoneyWithInputTx(CScript scriptPubKey, int64 nValue, int64 nNetFee, CWalletTx& wtxIn, CWalletTx& wtxNew, bool fAskFee)
{
    ...
    if (fAskFee && !uiInterface.ThreadSafeAskFee(nFeeRequired))
        return "ABORTED";
#else
    if (fAskFee && !ThreadSafeAskFee(nFeeRequired, "Runnercoin", NULL))
        return "ABORTED";
#endif
...
Value sendtoname(const Array& params, bool fHelp)
{
    ...
    if (!AddressToHash160(strAddress, hash160))
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "No valid runnercoin address");
...
CHooks* InitHook()
{
    ...
    hashGenesisBlock = hashRunnercoinGenesisBlock[fTestNet ? 1 : 0];
    printf("Setup runnercoin genesis block %s\n", hashGenesisBlock.GetHex().c_str());
    return new CRunnercoinHooks();
}

bool CRunnercoinHooks::IsStandard(const CScript& scriptPubKey)
{
    return true;
}
...
void CRunnercoinHooks::AddToWallet(CWalletTx& wtx)
{
}

bool CRunnercoinHooks::IsMine(const CTransaction& tx)
{
    if (tx.nVersion != NAMECOIN_TX_VERSION)
        return false;
...
bool CRunnercoinHooks::IsMine(const CTransaction& tx, const CTxOut& txout, bool ignore_name_new /* = false*/)
{
    if (tx.nVersion != NAMECOIN_TX_VERSION)
        return false;
...
void CRunnercoinHooks::AcceptToMemoryPool(CTxDB& txdb, const CTransaction& tx)
{
    if (tx.nVersion != NAMECOIN_TX_VERSION)
        return;
...
void CRunnercoinHooks::RemoveFromMemoryPool(const CTransaction& tx)
{
    if (tx.nVersion != NAMECOIN_TX_VERSION)
        return;
...
bool CRunnercoinHooks::ConnectInputs(CTxDB& txdb,
        map<uint256, CTxIndex>& mapTestPool,
        const CTransaction& tx,
        vector<CTransaction>& vTxPrev,
        vector<CTxIndex>& vTxindex,
...
bool CRunnercoinHooks::DisconnectInputs(CTxDB& txdb,
        const CTransaction& tx,
        CBlockIndex* pindexBlock)
{
    if (tx.nVersion == GAME_TX_VERSION)
...
bool CRunnercoinHooks::CheckTransaction(const CTransaction& tx)
{
    if (tx.nVersion != NAMECOIN_TX_VERSION)
        return true;
...
bool CRunnercoinHooks::ExtractAddress(const CScript& script, string& address)
{
...
bool CRunnercoinHooks::ConnectBlock(CBlock& block, CTxDB& txdb, CBlockIndex* pindex, int64 &nFees, unsigned int nPosAfterTx)
{
...
bool CRunnercoinHooks::DisconnectBlock(CBlock& block, CTxDB& txdb, CBlockIndex* pindex)
{
    RollbackGameState(txdb, pindex);
    return true;
}

bool CRunnercoinHooks::GenesisBlock(CBlock& block)
{
    block = CBlock();
...
int CRunnercoinHooks::LockinHeight()
{
        return 0;
}

bool CRunnercoinHooks::Lockin(int nHeight, uint256 hash)
{
    return true;
}

string CRunnercoinHooks::IrcPrefix()
{
    return "runnercoin";
}
