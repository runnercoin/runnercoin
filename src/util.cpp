...
#else
    const char* pszModule = "runnercoin";
#endif
    if (pex)
...
string GetDefaultDataDir()
{
    ...
    // Windows: C:\Documents and Settings\username\Application Data\Runnercoin
    // Mac: ~/Library/Application Support/Runnercoin
    // Unix: ~/.runnercoin
#ifdef __WXMSW__
    // Windows
...
{
    return GetConfigFile("runnercoin.conf");
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    {
        // Don't overwrite existing settings so command line settings override runnercoin.conf
...
{
    namespace fs = boost::filesystem;
    fs::path pathConfig(GetArg("-pid", "runnercoind.pid"));
    if (!pathConfig.is_complete())
        pathConfig = fs::path(GetDataDir()) / pathConfig;
    return pathConfig.string();
}
